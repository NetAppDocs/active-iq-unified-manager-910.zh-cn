...
NOTE: 在 VMware vSphere 系统上安装 Unified Manager 时， MySQL 3306 的默认端口仅限于本地主机。这不会影响保留先前配置的任何升级情形。可以修改此配置，并可通过维护控制台将此连接提供给其他主机。
...
[NOTE]
====
用于 HTTP 和 HTTPS 通信的端口（端口 80 和 443 ）可以使用 Unified Manager 维护控制台进行更改。有关详细信息，请参见 link:../config/concept_configure_unified_manager.html["正在配置 Active IQ Unified Manager"]。
